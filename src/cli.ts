...
function printUsage(out: (msg: string) => void): void {
  ...
  out("  list     List tasks");
  out("  search   Search tasks by title or description");
  ...
  out("");
  out("Arguments for search:");
  out("  <query>     Case-insensitive text to search in title and description");
}
...
export async function runCLI(
  ...
  switch (command) {
    ...
    case "rm":
      ...
      break;
    case "search":
      await runSearch(positional[0], { stdout, stderr, exit });
      break;
    default:
